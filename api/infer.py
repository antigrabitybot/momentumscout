...
処理フロー (handoff/02 §6-1):
  ...
  2. X recent search: 直近48hの投稿本文 最大20件 (失敗時は空配列で縮退)
  2b. X Counts:Recent: 直近7日の投稿数の日別推移 (Posts:Readを消費しない
      定額$0.005/回の集計専用API。失敗時は空配列で縮退)
...
def fetch_post_count_trend(name: str, code: str) -> list[dict]:
    """X投稿数の日別推移 (直近7日、Counts:Recentエンドポイント)。
    検索本文は取得しない集計専用APIのため定額$0.005/回で済み、
    fetch_x_posts の48h・最大20件という取得件数の制約を受けずに
    実際の話題化の推移(急拡大/沈静化)を安価に把握できる。失敗時は空配列で縮退"""
    token = os.environ.get("X_BEARER_TOKEN")
    if not token:
        return []
    query = f'("{name}" OR {code}) lang:ja -is:retweet'
    url = ("https://api.x.com/2/tweets/counts/recent?"
           + urllib.parse.urlencode({"query": query, "granularity": "day"}))
    req = urllib.request.Request(url, headers={"Authorization": f"Bearer {token}"})
    try:
        with urllib.request.urlopen(req, timeout=10) as res:
            rows = json.loads(res.read().decode()).get("data", [])
    except Exception as e:  # noqa: BLE001
        print(f"投稿数推移取得縮退: {e}", file=sys.stderr)
        return []
    out = []
    for r in rows:
        start = r.get("start", "")
        try:  # UTC日付→JST日付相当に丸める(時刻部分は集計粒度がdayのため厳密変換は不要)
            d = datetime.strptime(start[:10], "%Y-%m-%d").date()
        except ValueError:
            continue
        out.append({"date": d.isoformat(), "count": r.get("tweet_count", 0)})
    return out
...
def build_evidence(body: dict, ir: list, x_posts: list, x_trend: list) -> dict:
    """handoff/01 §3 の evidenceパック構造 (x_post_trend_7d はユーザー要望で追加)"""
    return {
        ...
        "ir_recent7d": ir,
        "x_posts_48h": x_posts,
        "x_post_trend_7d": x_trend,  # 日別投稿件数の推移 (話題化が拡大/沈静化しているかの判断材料)
...
def _balanced_json_blocks(text: str):
    """各 { を起点に括弧の対応が取れた完全なJSONブロックを順に返す
    (前後の文章・複数オブジェクト・末尾の余計な文章を許容)"""
    start = text.find("{")
    while start != -1:
...
def extract_json(text: str) -> dict:
    """モデル出力から回答JSONを抽出。primary_cause を含むオブジェクトを優先し、
    コードフェンス・前後の文章・文字列内の生改行などの崩れを許容する"""
    parsed = []
    last: Exception = ValueError("JSONが見つからない")
    candidates = list(_balanced_json_blocks(text))
    m = re.search(r"\{.*\}", text, re.S)  # 旧来の貪欲マッチも保険で試す
    if m and m.group(0) not in candidates:
        candidates.append(m.group(0))
    for cand in candidates:
        for strict in (True, False):  # strict=False は文字列内の生改行等を許容
            try:
                o = json.loads(cand, strict=strict)
                ...
                last = e
    for o in parsed:
        if "primary_cause" in o:  # 回答スキーマのオブジェクトを優先
            return o
    if parsed:
        return parsed[0]
...
def _messages_api(payload: dict, api_key: str, timeout: int) -> str:
    req = urllib.request.Request(
...
def anthropic_call(evidence: dict, api_key: str) -> str:
    """推論本体。モデルの生テキストを返す"""
    # Vercel maxDuration=60s に対し web_search 2回分の余裕を見る
    return _messages_api({
        "model": MODEL,
...
FIX_JSON_PROMPT = (
    "あなたはJSON修復器です。入力テキストに含まれる回答オブジェクトを、"
    "有効なJSONとして出力してください。キーは primary_cause(文字列), "
    "confidence(高/中/低), secondary(文字列またはnull), sns_heat(文字列またはnull), "
    "sources(type/date/title/urlを持つ配列), note(文字列またはnull)。"
    "内容の追加・変更・要約はせず、構文だけを修復すること。JSON以外を出力しないこと。")


def fix_json(broken_text: str, api_key: str) -> dict:
    """壊れたモデル出力をHaikuで構文修復する縮退パス (ツールなし・低コスト)"""
    text = _messages_api({
        "model": "claude-haiku-4-5",
        "max_tokens": MAX_TOKENS,
        "system": FIX_JSON_PROMPT,
        "messages": [{"role": "user", "content": broken_text[:8000]}],
    }, api_key, timeout=25)
    return extract_json(text)


def call_claude(evidence: dict, api_key: str) -> dict:
    """パース失敗はJSON修復で縮退し、スキーマ不一致なら1回だけ再試行
    (handoff/02 §6-1 手順5)"""
    last_err = ""
    for _ in range(2):
        try:
        ...
        if err is None:
            return result
        last_err = f"スキーマ不一致: {err} (keys={sorted(result)[:10]})"
...
class handler(BaseHTTPRequestHandler):  # noqa: N801  Vercel規約のクラス名
    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            payload = {"error": "推論に失敗しました。時間をおいて再試行してください"}
    ...
    def _handle(self) -> None:
        ...
        x_trend = fetch_post_count_trend(name, code)
        evidence = build_evidence(body, ir, x_posts, x_trend)
